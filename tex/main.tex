...
\def\dissertationtitle{Maximising throughput between a Risc-V microprocessor and
  linear algebra co-processor.}
...
% \section{Risc-V Vector Instructions}
% TODO: Not fully finalised --- poor compiler support
...
data must be aligned appropriately. A 32-bit AXI4 bus may only access an aligned
4-byte block per request. As a consequence, it is possible to fail
...
We make use of a floating-point fused multiply-accumulate module. These are used
...
The simplest functional unit should take $11N$~cycles per element for an
$N$~element vector. 
...
result using a floating point adder, we observe that addition will take $4N$
...
% Will not work (peculiarities with instruction \$)
